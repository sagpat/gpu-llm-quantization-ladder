import torch, time, os
from transformers import AutoModelForCausalLM, AutoTokenizer

model_id = "mistralai/Mistral-7B-Instruct-v0.2"
token = os.environ.get("HF_TOKEN")
tokenizer = AutoTokenizer.from_pretrained(model_id, token=token)

print("Loading Mistral 7B in INT8...")
model = AutoModelForCausalLM.from_pretrained(
    model_id, load_in_8bit=True, device_map="cuda", token=token
)

mem = torch.cuda.memory_allocated()/1e9
print(f"Memory used: {mem:.2f} GB")
print(f"Memory saved vs FP16 baseline (~14GB): {14.0 - mem:.1f} GB saved")

questions = [
    ("What is the capital of France?",        "Paris"),
    ("What is 15 multiplied by 8?",           "120"),
    ("Who wrote Romeo and Juliet?",           "Shakespeare"),
    ("What planet is closest to the sun?",   "Mercury"),
    ("What is the chemical symbol for gold?", "Au"),
]

correct = 0
latencies = []
for q, expected in questions:
    inputs = tokenizer(f"Answer in one word or number: {q}",
                       return_tensors="pt").to("cuda")
    start = time.time()
    with torch.no_grad():
        out = model.generate(**inputs, max_new_tokens=15, do_sample=False)
    lat = (time.time()-start)*1000
    latencies.append(lat)
    ans = tokenizer.decode(out[0], skip_special_tokens=True)
    got = expected.lower() in ans.lower()
    if got: correct += 1
    print(f"Q: {q}")
    print(f"A: {ans[-60:].strip()} | Expected: {expected} | Correct: {got} | {lat:.0f}ms")

print(f"INT8 Accuracy: {correct}/5 = {correct/5*100:.0f}%")
print(f"Avg latency: {sum(latencies)/len(latencies):.0f}ms")
print(f"Peak memory: {torch.cuda.max_memory_allocated()/1e9:.2f} GB")